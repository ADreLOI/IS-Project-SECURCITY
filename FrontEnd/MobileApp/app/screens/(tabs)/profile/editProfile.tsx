      ...
      if(token)
      {
        ...
        try
        {
          const response = await axios.put(
            `http://10.0.2.2:3000/api/v1/cittadino/addContattiEmergenza/${decoded.id}`,
            { contattiEmergenza },
            {
              headers: {
  ...
  const editContattoEmergenza = async () =>
    {
      ...
      if(token)
      {
        ...
        if (contattoToEdit)
        {
          ...
          {
            const response = await axios.put(
              `http://10.0.2.2:3000/api/v1/cittadino/editContattiEmergenza/${decoded.id}`,
  ...
  const deleteContattoEmergenza = async () =>
    {
      ...
      if(token)
      {
        ...
        if(contattoToDelete)
        {
          ...
          {
            const response = await axios.put(
              `http://10.0.2.2:3000/api/v1/cittadino/deleteContattiEmergenza/${decoded.id}`,
              { idContatto },
              {
                headers: {
                  Authorization: `Bearer ${token}`
    ...
    else
    {
      ...
      if(token)
      {
        ...
        try
        {
          ...
          const response = await axios.put(
            `http://10.0.2.2:3000/api/v1/cittadino/editProfile/${decoded.id}`,
            { username,
              email
            },
            {
